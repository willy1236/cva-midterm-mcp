...
- `host/server.py` 的 `/chat` 流程會把使用者訊息送入 OpenAI，再依工具呼叫結果回傳最終回應。
- Host 層內還包含以下子模組：
...
- `AGENT_RESPONSE` 用於驗證助理輸出。
- `PEER_REVIEW` 與 `AUDIT_REPORT` 則對應後續可擴充的治理資料格式。

### 1.5 Audit / Logging

- `host/audits/governance_logger.py` 提供 `GovernanceLogger`。
- 目前記錄的動作類型包括 `TOOL_CALL_ALLOWED`、`TOOL_CALL_REJECTED`、`OUTPUT_VALIDATION_PASS`、`OUTPUT_VALIDATION_FAIL` 與 `POLICY_VIOLATION`。
...
- `host_flow_cli.py` 提供互動式 CLI，用來操作 host server、建立 session 與送出聊天訊息。
